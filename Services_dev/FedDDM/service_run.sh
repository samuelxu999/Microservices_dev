...
if  [ "start" == "$OPERATION" ] ; then
	...
	# run geth as node
	./docker_exec.sh ddm-service docker /home/docker/geth_cmd/startnode.sh &>/dev/null &

	# run sshd to start ssh server
	./docker_exec.sh ddm-service root /home/docker/geth_cmd/sshd_start.sh &>/dev/null &
